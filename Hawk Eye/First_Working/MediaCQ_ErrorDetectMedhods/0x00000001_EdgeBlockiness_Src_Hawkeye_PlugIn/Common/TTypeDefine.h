...
// VED & RED 통합에 필요한 정의
#define STR_TYPE_VEDS	 _T("VEDS")
#define STR_TYPE_REDS	 _T("REDS")
#define STR_PARAM_PIPE	 _T("-pipename")
#define STR_PARAM_WINDOW _T("-windowtitle")
#define STR_PARAM_TYPE	 _T("-servertype")

#define MAX_SIZE_JOBID    40
#define MAX_SIZE_TIMECODE 50
...
#define CQ_QC_VCR_HEADER_BLOCK    0x00000002 // QC 2번, 테잎손상이나 VCR Header의 과열로 인해 발생되는 에러 검출.
...
#define CQ_QC_BLOCK_STRIPE        0x00000400 // Block Stripe, Same Block Bar
...
#define CQ_QC_HAZARDOUS_CTS       0x00400000 // Hazardous Image Candidate, For KT
#define CQ_QC_BLURNESS            0x00400000 // Blurness
...
#define CQ_MAKE_BITMAP    ( CQ_VIDEO_CHECK|CQ_QC_AUDIO_LEVEL|CQ_QC_AU_MUTE|CQ_QC_AUDIO_ATTACK )
...
enum _EXECTION_TYPE{ _eUnknown, 
					 ...
					 _eAC3    ,
					 _eM4A    ,
					 _e3GP		// added by Tem (2011.11.04) - .3gp media Support.
};
...
#define _EXT_AC3   _T("AC3")
#define _EXT_M4A   _T("M4A")
#define _EXT_3GP   _T("3GP")	// added by Tem (2011.11.04) - .3gp media Support.
...
typedef struct tagAVResult
{
	...
	LPVOID               lpData                    ;//Fix for result image.
	DWORD                dwDataSize                ;
	LPBITMAPINFOHEADER   lpBih                     ;
	UINT                 nFrameStart               ;
	UINT                 nFrameEnd                 ;
	WORD			     wAudioChannel             ;//Current channel number
...
typedef struct _tagOrgResolutionSize
{
	DWORD dwOrgHeight	;
	DWORD dwOrgWidth	;
}ORGRESOLUTIONSIZE, *LPORGRESOLUTIONSIZE;
...
typedef struct tagDShowMediaInfo
{
	...
	DOUBLE    dMediaStartTime               ;//0프레임에서의Directshow 시간
...
//HD,SD를 판별 후 nSimultaneusProcess과 비교하여 작은 수로 등록
typedef struct tagProcessSplitInfo
{
	int nSimultaneusProcess;//시스템이 분할 될 수 있는 갯수.
	int nSplitCountHD      ;//HD가 분할 되어야 하는 갯수
	int nSplitCountSD      ;//SD가 분할 되어야 하는 갯수
}PROCESSSPLITINFO, *LPPROCESSSPLITINFO;

typedef struct tagCheckTime
{
	LONGLONG llStartTime;
	LONGLONG llEndTime  ;
} CHECKTIME, *LPCHECKTIME;

class CCheckTime : public CHECKTIME
{
public:
	CCheckTime(){
		memset( this, 0, sizeof(tagCheckTime) );
	};
	CCheckTime(const CHECKTIME& op){
		llStartTime  = op.llStartTime ;
		llEndTime    = op.llEndTime;
	};
	CCheckTime(const CCheckTime& op){
		llStartTime  = op.llStartTime ;
		llEndTime    = op.llEndTime;
	};
	CCheckTime(const LONGLONG& llStart,
			   const LONGLONG& llEnd  ){
		llStartTime  = llStart;
		llEndTime    = llEnd  ;
	};
...
public:
	void operator =(const CHECKTIME& op){
		llStartTime  = op.llStartTime ;
		llEndTime    = op.llEndTime;
	};
	void operator =(const CCheckTime& op){
		llStartTime  = op.llStartTime ;
		llEndTime    = op.llEndTime;
	};
};

typedef list<CCheckTime> LIST_CHECKTIME, *LPLIST_CHECKTIME;
typedef LIST_CHECKTIME::iterator LIST_CHECKTIMEIter;

#define INVALID_REFERENCE_TIME (-1)
#define INVALID_DOUBLE_TIME    (-1.0)
//}}kiokahn, check split file
