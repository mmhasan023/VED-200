...
BOOL CRegistry::CreateKey (HKEY hKeyRoot, LPCTSTR pszPath)
{
	...
	LONG  ReturnValue = RegCreateKeyEx (hKeyRoot,
		                                pszPath,
										0L,
										NULL,
...
BOOL CRegistry::Open (HKEY hKeyRoot, LPCTSTR pszPath, REGSAM samDesired)
{
	...
	LONG ReturnValue = RegOpenKeyEx(hKeyRoot,
		                            pszPath,
									0L,
									samDesired,
									&m_hKey);
...
BOOL CRegistry::Write (LPCTSTR pszKey, long lVal)
{
	DWORD dwValue;
...
BOOL CRegistry::Write (LPCTSTR pszKey, LPCTSTR pszData)
{
	...
	ASSERT(AfxIsValidAddress(pszData, lstrlen(pszData), FALSE));

	int nLength = lstrlen(pszData)*sizeof(TCHAR) + 1;
	LONG ReturnValue = RegSetValueEx (m_hKey, pszKey, 0L, REG_SZ,
		(CONST BYTE*) pszData, nLength );

	m_Info.lMessage = ReturnValue;
	m_Info.dwSize = nLength;
...
BOOL CRegistry::Read(LPCTSTR pszKey, long& lVal)
{
	ASSERT(m_hKey);
	ASSERT(pszKey);
...
/*// 2002.09.27. - Younghyun Ha
#ifdef BUTTON_LIST
#else
	ASSERT(pszKey);
#endif
// 2002.09.27. - Younghyun Ha - End*/

	DWORD dwType;

	// 2007.06.29 by seal - 용량 늘일 필요 있음
	DWORD dwSize = 255*10;
	TCHAR  szString[255*10];
	//CString szString;


	int nLength = lstrlen(sVal)*sizeof(TCHAR) + 1;
	LONG lReturn = RegQueryValueEx (m_hKey,
		                             pszKey,
									NULL,
									&dwType,
									(BYTE *)(LPCTSTR ) szString, //(BYTE *) szString,
									&dwSize);
...
BOOL CRegistry::Read (LPCTSTR pszKey, CByteArray& bcArray)
{
	...
	ASSERT(pszKey);
	const int iMaxChars = 4096;
	INT_PTR pOldSize = bcArray.GetSize();
	DWORD dwType;
	DWORD dwData = iMaxChars;
	BYTE* byData = (BYTE*)::calloc(iMaxChars, sizeof(TCHAR));
	ASSERT(byData);
...
BOOL CRegistry::Read (LPCTSTR pszKey, CDWordArray& dwcArray)
{
	...
	ASSERT(pszKey);
	const int iMaxChars = 4096;
	INT_PTR pOldSize = dwcArray.GetSize();
	DWORD dwType;
	DWORD dwData = iMaxChars;
	BYTE* byData = (BYTE*)::calloc(iMaxChars, sizeof(TCHAR));
	ASSERT(byData);
...
BOOL CRegistry::Read (LPCTSTR pszKey, CWordArray& wcArray)
{
	...
	ASSERT(pszKey);
	const int iMaxChars = 4096;
	INT_PTR pOldSize = wcArray.GetSize();
	DWORD dwType;
	DWORD dwData = iMaxChars;
	BYTE* byData = (BYTE*)::calloc(iMaxChars, sizeof(TCHAR));
	ASSERT(byData);
...
BOOL CRegistry::Read (LPCTSTR pszKey, CStringArray& scArray)
{
	...
	ASSERT(pszKey);
	const int iMaxChars = 4096;
	INT_PTR pOldSize = scArray.GetSize();
	DWORD dwType;
	DWORD dwData = iMaxChars;
	BYTE* byData = (BYTE*)::calloc(iMaxChars, sizeof(TCHAR));
	ASSERT(byData);
...
BOOL CRegistry::DeleteValue (LPCTSTR pszPath, LPCTSTR pszValue)
{
	ASSERT(m_hKey);
	...
	// open Regstry Key
	LONG lResult = RegOpenKeyEx(m_hKey, pszPath, 0, KEY_ALL_ACCESS, &hKey);

	if(lResult != ERROR_SUCCESS)
	{
		CString szErr = _T("Register Open Error\n");
		szErr +=pszPath;
		szErr +=pszValue;
		AfxMessageBox(szErr);
	}  

	lResult = RegDeleteValue(hKey, pszValue);        // 삭제
...
BOOL CRegistry::DeleteKey(HKEY hKeyRoot, LPCTSTR pszPath)
{
	ASSERT(pszPath);
	ASSERT(hKeyRoot);
...
void CRegistry::DisplayGetLastErrorMessage(void)
{
	LPVOID lpMsgBuf;
	FormatMessage( 
...
BOOL CRegistry::DeleteValue (HKEY hRootKey, LPCTSTR pszPath, LPCTSTR pszValue)
{
	ASSERT(hRootKey);
	...
	// open Regstry Key
	LONG lResult = RegOpenKeyEx(hRootKey, pszPath, 0, KEY_ALL_ACCESS, &hKey);
